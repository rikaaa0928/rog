...
pub fn parse_size(s: &str) -> Result<u64> {
    ...
    if s.ends_with('%') {
        ...
        let percent: f64 = percent_str
            .parse()
            .map_err(|_| Error::new(ErrorKind::InvalidInput, "invalid percentage format"))?;
    ...
    let num: u64 = num_str
        .trim()
        .parse()
        .map_err(|_| Error::new(ErrorKind::InvalidInput, "invalid number format"))?;
...
fn get_total_memory() -> Result<u64> {
    ...
    if let Ok(limit) = std::fs::read_to_string("/sys/fs/cgroup/memory/memory.limit_in_bytes") {
        if let Ok(bytes) = limit.trim().parse::<u64>() {
            if bytes < 9223372036854771712 {
                // Some high value indicating no limit
                return Ok(bytes);
            }
        }
    }
    ...
    Err(Error::new(
        ErrorKind::Other,
        "could not determine total memory",
    ))
}
