...
pub struct PbHttpServerReadHalf {
...
pub struct PbHttpServerWriteHalf {
...
pub struct PbHttpServerRunStream {
    reader: Arc<Mutex<RecvStream>>,
    writer: Arc<Mutex<SendStream<Bytes>>>,
    frame_buf: BytesMut,
    cache: Vec<u8>,
...
impl PbHttpServerRunStream {
    pub fn new(reader: RecvStream, writer: SendStream<Bytes>, options: PbHttpOptions) -> Self {
        Self {
    ...
    pub async fn handshake(&mut self, pw: &str) -> std::io::Result<Option<(RunAddr, String)>> {
        ...
        let req: Option<StreamReq> =
            read_message(&mut r, &mut self.frame_buf, self.options.max_message_size).await?;
...
async fn read_payload(
    ...
    *cache_pos = 0;
    ...
    let msg: Option<StreamReq> = read_message(&mut r, frame_buf, options.max_message_size).await?;
...
async fn write_payload(
    ...
    send_message(&mut w, &res, options.send_chunk_size).await
}

#[async_trait::async_trait]
impl RunReadHalf for PbHttpServerReadHalf {
...
#[async_trait::async_trait]
impl RunWriteHalf for PbHttpServerWriteHalf {
...
#[async_trait::async_trait]
impl RunStream for PbHttpServerRunStream {
