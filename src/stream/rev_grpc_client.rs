...
pub struct RevGrpcClientReadHalf {
    reader: Arc<Mutex<Streaming<RevStreamRes>>>,
    cache: Vec<u8>,
    cache_pos: usize,
}

pub struct RevGrpcClientWriteHalf {
    writer: Sender<RevStreamReq>,
}

pub struct RevGrpcClientRunStream {
    reader: Arc<Mutex<Streaming<RevStreamRes>>>,
    writer: Sender<RevStreamReq>,
    cache: Vec<u8>,
    cache_pos: usize,
...
impl RunReadHalf for RevGrpcClientReadHalf {
...
#[async_trait::async_trait]
impl RunWriteHalf for RevGrpcClientWriteHalf {
    async fn write(&mut self, buf: &[u8]) -> std::io::Result<()> {
        let req = RevStreamReq {
...
impl RevGrpcClientRunStream {
    pub fn new(reader: Arc<Mutex<Streaming<RevStreamRes>>>, writer: Sender<RevStreamReq>) -> Self {
        Self {
            reader,
            writer,
            cache: Vec::new(),
...
#[async_trait::async_trait]
impl RunStream for RevGrpcClientRunStream {
    ...
    fn split(self: Box<Self>) -> (Box<dyn RunReadHalf>, Box<dyn RunWriteHalf>) {
        (
            Box::new(RevGrpcClientReadHalf {
                reader: Arc::clone(&self.reader),
                cache: Vec::new(),
                cache_pos: 0,
            }),
            Box::new(RevGrpcClientWriteHalf {
                writer: self.writer,
            }),
        )
    }
