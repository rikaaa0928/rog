...
mod tests {
    ...
    // }
    #[test]
    fn test_url_parse_behavior() {
        use url::Url;

        // Case 1: "http://" + "/version" -> "http:///version"
        // This is the behavior observed in the issue.
        // The third slash makes "version" be interpreted as the host (authority), not the path.
        let dst_raw = "/version";
        let dst = format!("http://{}", dst_raw);
        assert_eq!(dst, "http:///version");
        ...
        // Case 2: "http://" + "version" -> "http://version"
        // Standard behavior where "version" is host.
        let dst_raw_2 = "version";
        let dst_2 = format!("http://{}", dst_raw_2);
        assert_eq!(dst_2, "http://version");

        let url_2 = Url::parse(&dst_2).unwrap();
        assert_eq!(url_2.host_str(), Some("version"));
        assert_eq!(url_2.path(), "/");
    }
}
