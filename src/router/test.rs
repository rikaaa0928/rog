...
mod tests {
    ...
    async fn test_resolve_ip_with_specific_dns() {
        ...
        let result = resolver.resolve_ip_with_specific_dns("www.google.com", dns_config).await;
        assert!(result.is_ok());
    ...
    async fn test_resolve_ip_with_default_dns() {
        let resolver = Resolver::new();
        let result = resolver.resolve_ip_with_default_dns("www.example.com").await;
        assert!(result.is_ok());
        assert!(!result.unwrap().is_empty());
    }
    ...
    async fn test_resolve_ip_with_doh() {
        ...
        let result = resolver.resolve_ip_with_doh("www.example.com", dns_config).await;
        assert!(result.is_ok());
        assert!(!result.unwrap().is_empty());
    }
    ...
    async fn test_cache_expiration() {
        let resolver = Resolver::new();
        let dns_config = "8.8.8.8:53";
        resolver.resolve_ip_with_specific_dns("www.baidu.com", dns_config).await.unwrap();
    ...
    async fn test_negative_cache_expiration() {
        let resolver = Resolver::new();
        let dns_config = "8.8.8.8:53";
        resolver.resolve_ip_with_specific_dns("invalid-domain", dns_config).await.unwrap_err();
